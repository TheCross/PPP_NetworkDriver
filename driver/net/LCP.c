...
#define NET_LPC_DATALINK_FILENAME           CONCAT1(NET_LCP_DATALINK)
#define NET_LCP_DATALINKINCLUDE             CREATEINCLUDEFILEWPATH(NET_LCP_DATALINKPATH, NET_LPC_DATALINK_FILENAME)
#include NET_LCP_DATALINKINCLUDE

#define NET_LCP_DATALINK_FUNPREFIX          CONCAT2(net_, NET_LCP_DATALINK)
#define net_LCP_datalink_setIPRxCallback    CONCAT2(NET_LCP_DATALINK_FUNPREFIX, _setLCPRxCallback)
...
static void sendResponse(net_PPP_LCP_code_e code, uint8_t identifier, databuffer_basic_t *data);
...
void net_LCP_init(void)
{
  ...
  databuffer_create(&txDataBufferHeader, txDataHeader, NET_LCP_HEADER_LENGTH);
  databuffer_create(&txDataBuffer, txData, 64);
}
...
static void rxCallback(databuffer_basic_t *rxDataBuffer)
{
  ...
  uint16_t length = (((uint16_t)rxDataBuffer->data[2] << 8) | ((uint16_t)rxDataBuffer->data[3])) - NET_LCP_HEADER_LENGTH;
  
  databuffer_copy_partial(&txDataBuffer, 0, rxDataBuffer, NET_LCP_HEADER_LENGTH, length);
  ...
  switch (code) {
    case LCP_ConfigureRequest:
      ...
      sendResponse(LCP_ConfigureAck, identifier, &txDataBuffer);
      break;
