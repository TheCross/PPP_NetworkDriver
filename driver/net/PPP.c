...
#define NET_PPP_UARTPREFIX                  CONCAT2(NET_PPP_UARTTYPE, NET_PPP_UARTNUMBER)
#define NET_PPP_UARTINCLUDE                 CREATEINCLUDEFILEWPATH(NET_PPP_UARTPATH, NET_PPP_UARTPREFIX)
#define net_PPP_uart_init                   CONCAT2(NET_PPP_UARTPREFIX, _init)
#define net_PPP_uart_setRxFinishedCallback  CONCAT2(NET_PPP_UARTPREFIX, _setRxFinishedCallback)
#define net_PPP_uart_setTxFinishedCallback  CONCAT2(NET_PPP_UARTPREFIX, _setTxFinishedCallback)
#define net_PPP_uart_txByte                 CONCAT2(NET_PPP_UARTPREFIX, _txByte)
...
#define HASTOBEESCAPED(c)   ((c == NET_PPP_FLAG) || (c == NET_PPP_ESCAPE))
...
static void (*rxCallback_IP)(databuffer_basic_t *rxDataBuffer) = rxCallback_DUMMY;
static void (*rxCallback_LCP)(databuffer_basic_t *rxDataBuffer) = rxCallback_DUMMY;
...
void net_PPP_init(void)
{
  ...
  for (uint8_t i=0; i<NET_PPP_RX_PACKET_BUFFER_SIZE; i++)
    databuffer_create(&(rxDataBuffer[i]), rxBuffer[i], NET_PPP_MTU_MAX);
...
void net_PPP_loop(void)
{
  ...
  while (numberOfRxPackets > 0) {
    ...
    indexOfLastRxPacket = (indexOfLastRxPacket + 1) % NET_PPP_RX_PACKET_BUFFER_SIZE;
    numberOfRxPackets--;
  }
}
    ...
    switch (rxState) {
      ...
      case PPPrxState_Control:
        if (hasFlag) {
          ...
          rxProtocol[indexOfFirstEmptyPacket] = ((net_PPP_protocol_e)b) << 8;
      ...
      case PPPrxState_ProtocolH:
        if (hasFlag) {
          ...
          rxProtocol[indexOfFirstEmptyPacket] |= (net_PPP_protocol_e)b;
      ...
      case PPPrxState_Data:
        if (hasFlag) {
          ...
          crc16_fcs_byte(&rxFCScalc, NET_PPP_ADDRESS);
          crc16_fcs_byte(&rxFCScalc, NET_PPP_CONTROL);
          crc16_fcs_byte(&rxFCScalc, (rxProtocol[indexOfFirstEmptyPacket] >> 8) & 0x00FF);
          crc16_fcs_byte(&rxFCScalc, (rxProtocol[indexOfFirstEmptyPacket] >> 0) & 0x00FF);
          crc16_fcs_data(&rxFCScalc, rxDataBuffer[indexOfFirstEmptyPacket].data, rxDataBufferWriteIndex - 2);
          rxFCSvalue = rxFCScalc ^ 0xFFFF;
          rxFCS = ((crc16_t)rxDataBuffer[indexOfFirstEmptyPacket].data[rxDataBufferWriteIndex - 1] << 8) | (crc16_t)rxDataBuffer[indexOfFirstEmptyPacket].data[rxDataBufferWriteIndex - 2];
          //rxFCS = rxFCSvalue;

          if (rxFCS == rxFCSvalue) {
            // Received valid ppp-packet.
            rxDataBuffer[indexOfFirstEmptyPacket].length = rxDataBufferWriteIndex - 2;
            rxDataBuffer[indexOfFirstEmptyPacket].tot_length = rxDataBufferWriteIndex - 2;

            indexOfFirstEmptyPacket = (indexOfFirstEmptyPacket + 1) % NET_PPP_RX_PACKET_BUFFER_SIZE;
            numberOfRxPackets++;
          } else {
          ...
          crc16_fcs_byte(&rxFCScalc, (rxProtocol[indexOfFirstEmptyPacket] >> 8) & 0x00FF);
          crc16_fcs_byte(&rxFCScalc, (rxProtocol[indexOfFirstEmptyPacket] >> 0) & 0x00FF);
          crc16_fcs_data(&rxFCScalc, rxDataBuffer[indexOfFirstEmptyPacket].data, rxDataBufferWriteIndex);
          rxFCSvalue = rxFCScalc ^ 0xFFFF;
      ...
      case PPPrxState_FcsH:
        if (hasFlag) {
          ...
          rxDataBuffer[indexOfFirstEmptyPacket].length = rxDataBufferWriteIndex;
          rxDataBuffer[indexOfFirstEmptyPacket].tot_length = rxDataBufferWriteIndex;
